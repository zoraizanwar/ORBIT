...
test('Forecasting: Strict Epistemic Separation Invariant', () => {
  // 1. Predictions must carry epistemic_level = PREDICTED
  assert.equal(DEMO_FORECAST_FIXTURE.epistemic_level, 'PREDICTED');
  for (const pred of DEMO_FORECAST_FIXTURE.predictions) {
    assert.equal(pred.epistemic_level, 'PREDICTED');
  }

  // 2. Historical calibration observations must carry CALCULATED or OBSERVED (never PREDICTED)
  for (const obs of DEMO_HISTORICAL_SERIES) {
    assert.notEqual(obs.epistemic_level, 'PREDICTED');
    assert.equal(obs.epistemic_level, 'CALCULATED');
  }
});

test('Forecasting: Chronological Boundary & Future Horizon Invariants', () => {
  const lastHistoricalYear = Math.max(
    ...DEMO_HISTORICAL_SERIES.map((h) => new Date(h.acquisition_datetime).getFullYear())
  );

  // Predictions must strictly succeed historical training years
  for (const pred of DEMO_FORECAST_FIXTURE.predictions) {
    assert.ok(
      pred.target_year > lastHistoricalYear,
      `Target year ${pred.target_year} must be strictly greater than last historical year ${lastHistoricalYear}`
    );
    assert.ok(
      pred.target_year <= 2050,
      `Target year ${pred.target_year} must not exceed the 2050 scenario boundary`
    );
  }
});

test('Forecasting: Statistical Confidence & Uncertainty Intervals', () => {
  for (const pred of DEMO_FORECAST_FIXTURE.predictions) {
    if (pred.lower_bound !== null && pred.upper_bound !== null) {
      assert.ok(
        pred.lower_bound <= pred.predicted_value,
        `Lower bound (${pred.lower_bound}) must be <= predicted (${pred.predicted_value})`
      );
      assert.ok(
        pred.predicted_value <= pred.upper_bound,
        `Predicted (${pred.predicted_value}) must be <= upper bound (${pred.upper_bound})`
      );
    }
  }
});

test('Forecasting: Historical Backtesting Out-of-Sample Diagnostics', () => {
  const { backtest } = DEMO_FORECAST_FIXTURE;
  assert.equal(backtest.status, 'COMPLETED');
  assert.ok(backtest.splits.length >= 3);
  assert.ok(backtest.overall_metrics !== null);
  assert.ok(backtest.overall_metrics.mae >= 0.0);
  assert.ok(backtest.overall_metrics.rmse >= 0.0);
  assert.ok(
    backtest.overall_metrics.r_squared >= 0.0 && backtest.overall_metrics.r_squared <= 1.0
  );
});

test('Forecasting: Cryptographic Provenance Trace Invariant', () => {
  const { provenance } = DEMO_FORECAST_FIXTURE;
  assert.ok(provenance.provenance_hash_sha256);
  assert.equal(provenance.provenance_hash_sha256.length, 64);
  assert.equal(provenance.deterministic_pipeline, true);
  assert.equal(provenance.ai_interpretation_invoked, false);
});
