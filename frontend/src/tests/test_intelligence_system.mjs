...
const DEMO_INTELLIGENCE_FIXTURES = [
  {
    id: 'intel-demo-001',
...
test('Intelligence System: Multi-Indicator Urban Expansion Invariants', () => {
  const urbanIntel = DEMO_INTELLIGENCE_FIXTURES.find(
    (f) => f.intelligence_type === 'URBAN_EXPANSION'
  );

  assert.ok(urbanIntel, 'Urban expansion intelligence fixture must exist');
  assert.equal(urbanIntel.rule_id, 'RULE_MULTI_URBAN_EXPANSION_v1');
  assert.equal(urbanIntel.epistemic_level, 'CALCULATED');
  assert.equal(urbanIntel.evidence_strength, 'STRONG');
  assert.ok(urbanIntel.affected_area_km2 > 0, 'Affected area must be positive');

  // Verify supporting evidence edges
  const supportingEdges = urbanIntel.evidence_graph.edges.filter(
    (e) => e.relationship_type === 'SUPPORTS' || e.relationship_type === 'CORROBORATES'
  );
  assert.ok(supportingEdges.length >= 2, 'Must have at least 2 corroborating evidence streams');
});

test('Intelligence System: Contradictory Evidence Downgrades Strength to INSUFFICIENT', () => {
  const disputedIntel = DEMO_INTELLIGENCE_FIXTURES.find(
    (f) => f.status === 'CONTRADICTED'
  );

  assert.ok(disputedIntel, 'Contradicted intelligence fixture must exist');
  assert.equal(disputedIntel.evidence_graph.has_contradictions, true);
  assert.equal(disputedIntel.evidence_graph.contradiction_count, 1);
  assert.equal(disputedIntel.evidence_strength, 'INSUFFICIENT');

  const contradictEdge = disputedIntel.evidence_graph.edges.find(
    (e) => e.relationship_type === 'CONTRADICTS'
  );
  assert.ok(contradictEdge, 'Must have explicit CONTRADICTS edge');
});

test('Intelligence System: Spatial Infrastructure Proximity & Buffer Correlation', () => {
  const corridorIntel = DEMO_INTELLIGENCE_FIXTURES.find(
    (f) => f.spatial_context.intersects_road_corridor === true
  );

  assert.ok(corridorIntel, 'Corridor intersecting fixture must exist');
  assert.ok(corridorIntel.spatial_context.closest_road_name, 'Closest road name must be present');
  assert.ok(corridorIntel.spatial_context.distance_to_closest_road_m <= corridorIntel.spatial_context.road_corridor_buffer_m);
});

test('Intelligence System: Strict Chronological Temporal Ordering', () => {
  DEMO_INTELLIGENCE_FIXTURES.forEach((fixture) => {
    const t1 = new Date(fixture.temporal_context.start_date).getTime();
    const t2 = new Date(fixture.temporal_context.end_date).getTime();
    assert.ok(t1 < t2, `Start date (${fixture.temporal_context.start_date}) must precede end date (${fixture.temporal_context.end_date})`);
    assert.ok(fixture.temporal_context.interval_days > 0, 'Interval days must be positive');
  });
});
